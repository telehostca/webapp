...
class HostingOrder extends Model
{
    ...
    public static function invoicePaid($invoice_id)
    {
        $hosting_orders = HostingOrder::where('invoice_id', $invoice_id)->get();

        foreach ($hosting_orders as $order) {
            $order->status = 'Active';
            $order->save();

            if($order->plan_id)
            {
                $customer = Contact::find($order->contact_id);
                if($customer)
                {
                    $domain = $order->domain;
                    ...
                    if($domain)
                    {
                        if(empty($username))
                        {
                            ...
                            // if username is longer than 8 characters, truncate it
                    ...
                    if($order->plan_id)
                    {
                        $hosting_plan = HostingPlan::find($order->plan_id);
                        if($hosting_plan && !empty($plan->hosting_provider_id))
                        {
                            $plan = $hosting_plan->api_name;
                            $server = HostingServer::find($plan->hosting_provider_id);
                            if($server)
                            {
