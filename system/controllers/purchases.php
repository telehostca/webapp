...
switch ($action) {
    ...
    case 'edit-post':
        ...
        if ($msg == '') {
            ...
            $d = ORM::for_table('sys_purchases')->find($iid);
            if ($d) {
                $d->userid = $cid;
                $d->account = $u['account'];
                $d->date = $idate;
    ...
    case 'save':
        AppEvent::trigger('purchases/add-post/');
        $cid = _post('cid');
        ...
        if ($msg == '') {
            ...
            if (_post('purchase_id') !== '' && _post('purchase_id') !== '0') {
            ...
            AppEvent::trigger('add_invoice_posted');
            echo $invoiceid;
        } else {
            echo $msg;
        }
    ...
    case 'add-payment':
        ...
        $sid = $routes['2'];
        $d = ORM::for_table('sys_purchases')->find($sid);
        if ($d) {
            $itotal = $d['total'];
            $ic = $d['credit'];
            $np = $itotal - $ic;
...
<div class="modal-footer">
    ...
    case 'add-payment-post':
        ...
        if ($msg == '') {
            ...
            $i = ORM::for_table('sys_purchases')->find($iid);
            if ($i) {
                $pc = $i['credit'];
                $it = $i['total'];
                $dp = $it + $pc;
...
';
    ...
    case 'send_email':
        AppEvent::trigger('purchases/send_email/');
        $msg = '';
        $email = _post('toemail');
        $cc = _post('ccemail');
        $bcc = _post('bccemail');
