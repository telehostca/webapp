...
switch ($action) {
    ...
    case 'add-post':
        AppEvent::trigger('quotes/add-post/');
        $cid = _post('cid');
        //find user with cid
        ...
        if ($msg == '') {
            ...
            if ($tax != '') {
                $dt = ORM::for_table('sys_tax')->find($tax);
                $taxrate = $dt['rate'];
                $taxname = $dt['name'];
                $taxtype = $dt['type'];
            }
    ...
    case 'list-recurring':
        AppEvent::trigger('quotes/list-recurring/');
    ...
    case 'edit-post':
        AppEvent::trigger('quotes/edit-post/');
        ...
        if ($msg == '') {
            ...
            $discount_amount = _post('discount_amount');
            $discount_amount = Finance::amount_fix($discount_amount);
            $discount_type = _post('discount_type');
            $discount_value = '0.00';
           ...
           // $vtoken = _raid(10);
            ...
            $d = ORM::for_table('sys_quotes')->find($iid);
            if ($d) {
                $d->subject = $subject;
                $d->stage = _post('stage');
                $d->validuntil = _post('edate');
        ...
        break;
    case 'delete':
        AppEvent::trigger('quotes/delete/');
        $id = $routes['2'];
        if (APP_STAGE == 'Demo') {
            r2(
                U . 'accounts/list',
    ...
    case 'mark_draft':
        AppEvent::trigger('quotes/mark_draft/');
        $iid = _post('iid');
        $d = ORM::for_table('sys_quotes')->find($iid);
        if ($d) {
            $d->stage = 'Draft';
    ...
    case 'mark_delivered':
        AppEvent::trigger('quotes/mark_delivered/');
        $iid = _post('iid');
        $d = ORM::for_table('sys_quotes')->find($iid);
        if ($d) {
            $d->stage = 'Delivered';
    ...
    case 'mark_on_hold':
        AppEvent::trigger('quotes/mark_on_hold/');
        $iid = _post('iid');
        $d = ORM::for_table('sys_quotes')->find($iid);
        if ($d) {
            $d->stage = 'On Hold';
    ...
    case 'mark_accepted':
        AppEvent::trigger('quotes/mark_accepted/');
        $iid = _post('iid');
        $d = ORM::for_table('sys_quotes')->find($iid);
        if ($d) {
            $d->stage = 'Accepted';
    ...
    case 'mark_lost':
        AppEvent::trigger('quotes/mark_lost/');
        $iid = _post('iid');
        $d = ORM::for_table('sys_quotes')->find($iid);
        if ($d) {
            $d->stage = 'Lost';
    ...
    case 'mark_dead':
        AppEvent::trigger('quotes/mark_dead/');
        $iid = _post('iid');
        $d = ORM::for_table('sys_quotes')->find($iid);
        if ($d) {
            $d->stage = 'Dead';
    ...
    case 'mail_invoice_':
        ...
        $d = ORM::for_table('sys_quotes')->find($sid);
...
<div class="mb-3">
    ...
    case 'send_email':
        AppEvent::trigger('quotes/send_email/');
    ...
    case 'convert_invoice':
        AppEvent::trigger('quotes/convert_invoice/');
        $iid = _post('iid');
        $q = ORM::for_table('sys_quotes')->find($iid);
