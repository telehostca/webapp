...
switch ($path) {
                ...
                switch ($method) {
                    ...
                    case 'POST':
                        ...
                        if (empty($errors)) {
                            ...
                            if ($config['client_drive'] == '1' && !file_exists(
    ...
    case 'v3':
        ...
        $api_key = ApiKey::where('apikey', $api_key_value)->first();
        ...
        switch ($action)
        {
            ...
            case 'invoice-by-number':
                ...
                if($action == 'invoice-by-number')
                {
                    $invoice = Invoice::where('cn', $invoice_id)->first();
                }
                else
                {
                ...
                $invoice_data = [
                    'id' => $invoice->id,
                    'invoice_number' => $invoice->invoicenum.$invoice->cn,
                    'status' => $invoice->status,
                    'date' => $invoice->date,
                    'date_formatted' => date($config['df'], strtotime($invoice->date)),
                    'due_date' => $invoice->duedate,
                    'due_date_formatted' => date($config['df'], strtotime($invoice->duedate)),
                    'amount' => $invoice->total,
                    'amount_formatted' => formatCurrency($invoice->total, $config['home_currency']),
                    'currency' => $invoice->currency,
                    'notes' => $invoice->notes,
                    'terms' => $invoice->terms,
                    'items' => [],
                    'url' => BASE_URL . 'client/iview/' . $invoice->id.'/token_'.$invoice->vtoken,
                    'contact' => [],
                ];
                ...
                foreach($items as $item)
                {
                    $invoice_data['items'][] = [
                        ...
                        'quantity' => $item->qty,
                        'price' => $item->amount,
                        'price_formatted' => formatCurrency($item->amount, $config['home_currency']),
                        'total' => $item->total,
                        'total_formatted' => formatCurrency($item->total, $config['home_currency']),
                    ];
                }

                $contact = Contact::where('id', $invoice->userid)->first();
