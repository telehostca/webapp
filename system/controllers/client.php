...
switch ($action) {
    ...
    case 'buy-now':
        ...
        if ($slug !== '' && $slug !== '0') {
            ...
            if ($item) {
                ...
                $default_tax = Tax::where('is_default', 1)->first();
        ...
        if ($id && $tracking_id) {
            ...
            if ($order) {
                if($order->server_id)
                {
                    ...
                    if ($server && $server->type === 'cpanel' && $order->username) {
                        ...
                        if (!$api_call['success'] && $order->login_url) {
                            redirect_to($order->login_url);
                        }
                        if(!empty($api_call['response']['data']['url']))
                        {
                            header('Location: '.$api_call['response']['data']['url']);
                            exit;
                        }
                    }
                }

                redirect_to('client/view-order/' . $order->id);
            }
        }
    ...
    case 'shopping-cart-update-term':
        ...
        if ($shopping_cart && $shopping_cart->items) {
            ...
            if (
                ...
                {
                    $tax_rate = $default_tax->rate;
                    $tax_amount = ($item->$db_term * $tax_rate) / 100;
                }

                $items['hosting'][$item_id]['tax_rate'] = $tax_rate;
                $items['hosting'][$item_id]['tax_amount'] = $tax_amount;
    ...
    case 'checkout':
        ...
        if ($shopping_cart && ($shopping_cart && $shopping_cart->items)) {
        ...
        $default_tax = Tax::getDefault();


        \view(get_theme_file('checkout'), [
            ...
            if (!empty($items['domains'])) {
                foreach ($items['domains'] as $key => $value) {
                    ...
                    if ($domain_price) {
                        ...
                        $term = (int) $value['term'];
                        $term_name = $term === 1 ? '1 year' : $term . ' years';
                        $invoice_items[] = [
    ...
    case 'domain-register':
        ...
        \view(get_theme_file('domain_register'), [
    ...
    case 'hosting-orders':
        ...
        \view(get_theme_file('hosting_orders'), [
            'orders' => $orders,
    ...
    case 'view-order':
        ...
        if ($id !== '' && $id !== '0') {
            ...
            if ($order) {
                ...
                \view(get_theme_file('view_order'), [
                    'order' => $order,
    ...
    case 'iprint':
        AppEvent::trigger('client/iprint/');
    ...
    case 'ipay':
        ...
        AppEvent::trigger('client/ipay/pg', [$pg, $id, $token]);
        ...
        if ($d) {
            ...
            $u = ORM::for_table('crm_accounts')->find($d->userid);
            ...
            switch ($pg) {
                ...
                case 'manualpayment':
                    AppEvent::trigger('client/manualpayment/');
                ...
                case 'ccavenue':
                    ...
                    if ($p) {
                        require APP_SYSTEM_PATH . '/lib/misc/ccavenue.php';

                        $currency_code = $p['c2'];
                        $c3 = $p['c3'];

                        if ($c3 != '' && is_numeric($c3) && $c3 != '1') {
                            $amount /= $c3;
                        }
                        ...
                        require APP_SYSTEM_PATH . '/lib/misc/ccform.php';
                    }
    ...
    case 'ipay_cancel':
        AppEvent::trigger('client/ipay_cancel/');
    ...
    case 'ipay_submitted':
        AppEvent::trigger('client/ipay_submitted/');
    ...
    case 'ipay_ipn':
        AppEvent::trigger('client/ipay_success/');
        ...
        if ($d) {
            ...
            if(class_exists('HostingOrder'))
            {
                HostingOrder::invoicePaid($id);
            }
    ...
    case 'btpay_submitted':
        AppEvent::trigger('client/btpay_submitted/');
                ...
                if ($d) {
                    ...
                    if ($token != $vtoken) {
                        ...
                        if ($result->success) {
                            ...
                            //                            $a = ORM::for_table('sys_accounts')->where('id', $account)->first(); // get braintree balance
                            //                            $cbal = $a['balance']; // customer balance
                            //                            $nbal = $cbal + $amount;
                            //                            $a->balance = $nbal;
                            //                            $a->save(); // update customer balance
                            //                            $d = new Transaction(); // BOF add a transaction
    ...
    case 'ccsubmit':
        ...
        if ($p) {
            ...
            require APP_SYSTEM_PATH . '/lib/misc/ccsubmit.php';
        }

        break;

    case 'login':
        AppEvent::trigger('client/login/');
    ...
    case 'register':
        ...
        AppEvent::trigger('client/register/');
    ...
    case 'forgot_pw':
        AppEvent::trigger('client/forgot_pw/');
    ...
    case 'forgot_pw_post':
        AppEvent::trigger('client/forgot_pw_post/');
    ...
    case 'auto_login':
        AppEvent::trigger('client/auto_login/');
    ...
    case 'register_post':
        ...
        AppEvent::trigger('client/register_post/');
        ...
        AppEvent::trigger('client_register_post_data_posted');
    ...
    case 'company_summary':
        ...
        if ($user->cid) {
            $d = ORM::for_table('sys_companies')->find($user->cid);
...
<table class="table table-bordered">
   ...
   <tbody>
    ...
    case 'invoices':
        AppEvent::trigger('client/invoices/');
        ...
        view(get_theme_file('client_invoices'), [
    ...
    case 'quotes':
        AppEvent::trigger('client/quotes/');
        ...
        \view(get_theme_file('client_quotes'));

        break;

    case 'transactions':
        AppEvent::trigger('client/transactions/');
...
vMax: \'9999999999999999.00\',
    ...
    case 'profile':
        AppEvent::trigger('client/profile/');
        ...
        \view(get_theme_file('client_profile'));
    ...
    case 'profile_edit_post':
        AppEvent::trigger('client/profile_edit_post/');
        $c = Contacts::details();
        $id = $c->id;
    ...
    case 'logout':
        AppEvent::trigger('client/logout/');
        $c = Contacts::details();
    ...
    case 'dl':
        require APP_SYSTEM_PATH . '/lib/mime.php';
    ...
    case 'downloads':
        ...
        view(get_theme_file('client_downloads'));
 ...
 ';
    ...
    case 'pay_with_credit':
        ...
        if ($d) {
            ...
            if ($msg === '') {
                ...
                //                $a = ORM::for_table('sys_accounts')->where('account', $account)->first();
                //                $cbal = $a['balance'];
                //                $nbal = $cbal + $amount;
                //                $a->balance = $nbal;
                //                $a->save();
    ...
    case 'receipt':
        ...
        if ($transaction) {
            ...
            $currency_symbol = $currency ? $currency->symbol : $config['currency_code'];
            ...
            $tpl = $device == 'render' ? 'client_receipt_mobile' : 'client_receipt';
    ...
    case 'form':
        ...
        if ($lead_form) {
            $extend = $embed && $embed !== '' ? 'canvas' : 'paper';
    ...
    case 'tickets':
        ...
        switch ($req) {
            case 'new':
                ...
                $ui->assign('deps', $deps);
                view(get_theme_file('client_tickets_new'), []);
            ...
            case 'view':
                ...
                if ($d) {
                    ...
                    \view(get_theme_file('tickets_view') ,[
                        'admins' => $admins,
                    ]);
                } else {
            ...
            case 'all':
                ...
                view(get_theme_file('client_tickets_all'));
        ...
        .paper {
            ...
            case 'create_post':
                ...
                if ($config['recaptcha'] == 1 && !$_SESSION['recaptcha_verified']) {
                    if (
    ...
    case 'purchase_print':
        ...
        if ($d) {
            ...
            require APP_SYSTEM_PATH . '/lib/invoices/purchase_print.php';
        } else {
            r2(U . 'customers/list', 'e', $_L['Account_Not_Found']);
        }
    ...
    case 'uploads':
        ...
        \view(get_theme_file('client_uploads'), [
            'user' => $c,
            'files' => $files,
        ]);
    ...
    case 'payment-stripe':
        ...
        if ($invoice && $payment_gateway) {
            ...
            if ($charge->status !== null && $charge->status == 'succeeded') {
                $invoice->status = 'Paid';
                $invoice->save();
            }
    ...
    case 'products':
        ...
        switch ($selected_tab)
        {
            case 'hosting':
                ...
                $data['hosting_orders'] = HostingOrder::where('contact_id', $contact->id)
                    ->orderBy('id', 'desc')
                    ->get();
        ...
        $data['selected_navigation'] = 'products';
        $data['selected_tab'] = $selected_tab;
        $data['hosting_servers'] = HostingServer::all()
            ->keyBy('id')
            ->all();

        $data['hosting_plans'] = HostingPlan::all()
            ->keyBy('id')
            ->all();

        view('client_products', $data);
