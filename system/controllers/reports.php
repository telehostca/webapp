...
switch ($action) {
    ...
    case 'sales':
        ...
        if ($all_data) {
            $invoice_items = InvoiceItem::whereIn('invoiceid', $paid_invoices)->get();
        } else {
        ...
        foreach ($invoice_items as $invoice_item) {
            $total_sales += $invoice_item['qty'];
        }

        $total_revenue = 0;

        foreach ($invoice_items as $invoice_item) {
            $total_revenue += $invoice_item['qty'] * $invoice_item['price'];
        }

        $top_selling_product_id = 0;

        $top_selling_product_units = 0;

        $items_sold_by_id = [];
        foreach ($invoice_items as $invoice_item) {
            if(empty($invoice_item['item_id']))
            {
                continue;
            }
            if (isset($items_sold_by_id[$invoice_item['item_id']])) {
                $items_sold_by_id[$invoice_item['item_id']] += $invoice_item['qty'];
            } else {
                $items_sold_by_id[$invoice_item['item_id']] = $invoice_item['qty'];
            }
        }
        ...
        //Top selling product
        foreach ($items_sold_by_id as $item_id => $qty) {
            if ($qty > $top_selling_product_units) {
                $top_selling_product_id = $item_id;
                $top_selling_product_units = $qty;
            }
        }

        $top_selling_product_name = '--';

        if($top_selling_product_id){
            $top_selling_product = HostingPlan::find($top_selling_product_id);
        ...
        $last_year_invoice_paid_total = 0;

        $last_year_invoices = Invoice::where('aid', $user->id)
            ->where('status', 'Paid')
        ...
        foreach ($last_year_invoices as $last_year_invoice) {
            $last_year_invoice_paid_total += $last_year_invoice->total;
        }

        $sales_growth_rate = 0;

        if($last_year_invoice_paid_total){
            $sales_growth_rate = ($total_revenue - $last_year_invoice_paid_total) / $last_year_invoice_paid_total * 100;
        }
        ...
        view('reports_sales',[
            ...
            'top_selling_product_units' => $top_selling_product_units,
