...
const API_BASE_URL = 'https://jsonresp.test'; // Advanced PDF generation API
...
$_L = file_exists($ib_language_file_path) ? require $ib_language_file_path : 'system/i18n/en.php';

if (file_exists('system/overrides/i18n.php')) {
    $overrides_language_strings = require APP_SYSTEM_PATH . '/overrides/i18n.php';
    if(is_array($overrides_language_strings)){
...
const BASE_URL_ALT = APP_URL . '/?ng='; // Alternative URL for some cases
...
{
    if (isset($_SESSION['uid'])) {
        $d = ORM::for_table('user')->find($_SESSION['uid']);
...
require APP_BASE_PATH . '/ui/theme/default/functions.php';
...
function ui()
{
    ...
    $ui->registerPlugin('modifier', 'formatCurrency', 'formatCurrency');
    $ui->registerPlugin('modifier', 'explode', 'explode');
    $ui->registerPlugin('modifier', '__', '__');
    $ui->registerPlugin('modifier', 'sp_get_contact_image', 'sp_get_contact_image');
    $ui->registerPlugin('modifier', 'strtotime', 'strtotime');
    $ui->registerPlugin('modifier', 'predict_next_serial', 'predict_next_serial');
    $ui->registerPlugin('modifier', 'has_access', 'has_access');
    $ui->registerPlugin('modifier', 'date', 'date');
    $ui->registerPlugin('modifier', 'translate_date_string', 'translate_date_string');
    $ui->registerPlugin('modifier', 'ib_lan_get_line', 'ib_lan_get_line');
    $ui->registerPlugin('modifier', 'get_option', 'get_option');
    $ui->registerPlugin('modifier', 'ucfirst', 'ucfirst');
    $ui->registerPlugin('modifier', 'sp_config_or_default', 'sp_config_or_default');
    $ui->registerPlugin('modifier', 'md5', 'md5');
    $ui->registerPlugin('modifier', 'defined', 'defined');
    $ui->registerPlugin('modifier', 'get_or_default', 'get_or_default');
    $ui->registerPlugin('modifier', '_raid', '_raid');
    $ui->registerPlugin('modifier', 'getCurrencyConfigJson', 'getCurrencyConfigJson');
    $ui->registerPlugin('modifier', 'numberFormatUsingCurrency', 'numberFormatUsingCurrency');
    $ui->registerPlugin('modifier', 'sp_transform_string_template', 'sp_transform_string_template');
    $ui->registerPlugin('modifier', 'str_pad', 'str_pad');
    $ui->registerPlugin('modifier', 'ib_after_1_month', 'ib_after_1_month');
    $ui->registerPlugin('modifier', 'number_format', 'number_format');
    $ui->registerPlugin('modifier', 'urldecode', 'smarty_modifier_urldecode');
    $ui->registerPlugin('modifier', 'urlencode', 'smarty_modifier_urlencode');
    $ui->registerPlugin('modifier', 'getAdminImage', 'getAdminImage');
    $ui->registerPlugin('modifier', 'strtolower', 'strtolower');
    $ui->registerPlugin('modifier', 'base64_encode', 'base64_encode');
    $ui->registerPlugin('modifier', 'str_replace', 'str_replace');
    $ui->registerPlugin('modifier', 'inSession', 'inSession');
    $ui->registerPlugin('modifier', 'strTrunc', 'strTrunc');
    $ui->registerPlugin('modifier', 'floor', 'floor');
    $ui->registerPlugin('modifier', 'addslashes', 'addslashes');
    $ui->registerPlugin('modifier', 'getContactFormattedAddress', 'getContactFormattedAddress');
    $ui->registerPlugin('modifier', 'get_custom_field_value', 'get_custom_field_value');
    $ui->registerPlugin('modifier', 'gstIndiaSplitTaxValue', 'gstIndiaSplitTaxValue');
    $ui->registerPlugin('modifier', 'get_discount_amount', 'get_discount_amount');
    $ui->registerPlugin('modifier', 'ib_money_format', 'ib_money_format');
    $ui->registerPlugin('modifier', 'json_decode', 'json_decode');
    $ui->registerPlugin('modifier', 'ib_pg_count', 'ib_pg_count');
    $ui->registerPlugin('modifier', 'app_round', 'app_round');
    $ui->registerPlugin('modifier', 'csrf_token', 'csrf_token');
    $ui->registerPlugin('modifier', 'appMarkdownRender', 'appMarkdownRender');
    $ui->registerPlugin('modifier', 'csrf_field', 'csrf_field');
    $ui->registerPlugin('modifier', 'time', 'time');
    $ui->registerPlugin('modifier', 'getcwd', 'getcwd');
    $ui->registerPlugin('modifier', 'cloudonex_get_invoice_status_with_badge', 'cloudonex_get_invoice_status_with_badge');
    $ui->registerPlugin('modifier', 'cloudonex_get_order_status_with_badge', 'cloudonex_get_order_status_with_badge');
    $ui->registerPlugin('modifier', 'cloudonex_get_invoice_number', 'cloudonex_get_invoice_number');
    $ui->registerPlugin('modifier', 'cloudonex_get_ticket_status_with_badge', 'cloudonex_get_ticket_status_with_badge');
    $ui->registerPlugin('modifier', 'get_available_item_pricing_terms', 'get_available_item_pricing_terms');
    $ui->registerPlugin('modifier', 'format_number_using_currency_code', 'format_number_using_currency_code');

    $ui->registerClass('Contact', 'Contact');
    $ui->registerClass('Tax', 'Tax');
    $ui->registerClass('Transaction', 'Transaction');
    ...
    if (isset($_SESSION['notify']) || isset($_SESSION['success']) || isset($_SESSION['error'])) {
        $notify = $_SESSION['notify'] ?? null;
        $ntype = $_SESSION['notification_type'] ?? ($_SESSION['ntype'] ?? '');

        if(isset($_SESSION['success']))
        {
            $ntype = 'success';
            $notify = $_SESSION['success'];
...
{
    $d = AppConfig::where('setting', $option)
        ->first();
    if ($d) {
        return false;
    } else {
...
function add_css_admin($path)
{
    ...
    $plugin_ui_header_admin_css[] = $path;
}
...
function add_plugin_ui_header_client($header = '')
{
    ...
    $plugin_ui_header_client[] = $header;
}
...
function add_css_client($path)
{
    ...
    $plugin_ui_header_client_css[] = $path;
}
...
{
    $currency_code = $symbol == '' ? $config['currency_code'] : $symbol;
    ...
    $dec_digit = $currency_decimal_digits == 'true' ? 2 : 0;
...
function secondary_currency()
{
    ...
    if (count($c_check) == 2) {
        return ORM::for_table('sys_currencies')
...
function ib_js_date_format($format, $js = 'moment')
{
    if ($js == 'moment') {
        ...
        return str_replace('jS', 'Do', $format);
    } elseif (($js = 'picker') !== '') {
        $format = str_replace('d', 'dd', $format);
        $format = str_replace('m', 'mm', $format);
        $format = str_replace('Y', 'yyyy', $format);
        $format = str_replace('M', 'mmm', $format);
        return str_replace('jS', 'd', $format);
    } else {
        $format = str_replace('d', 'DD', $format);
        $format = str_replace('m', 'MM', $format);
        $format = str_replace('Y', 'YYYY', $format);
...
{
    $d = ORM::for_table('sys_users')->find($aid);
    if ($d) {
        return $d->fullname;
    } else {
        return '';
...
{
    $d = ORM::for_table('sys_companies')->find($company_id);
    if ($d) {
...
function namePresenter($name)
{
    ...
    return '<span class="ib_avatar ib_bg_' .
    $css_bg .
    '">' .
    $first_name_letter .
    $last_name_letter .
...
function updateCheck($purchase_key = '')
{
    ...
    if (json_last_error() === JSON_ERROR_NONE) {
        if (property_exists($resp, 'build') && $resp->build !== null) {
...
function apiAuth()
{
    $token = $_GET['api_key'] ?? getBearerToken();
...
{
    $contact = false;
    $token = isset($_GET['api_key']) ? $_GET['api_key'] : getBearerToken();
...
function getBalances($user)
{
    ...
    $transactions = $all_data ? Transaction::all() : Transaction::where('aid', $user->id)->get();
    ...
    $banks = $all_data ? Account::all() : Account::where('owner_id', $user->id)->get();
    ...
    foreach ($transactions as $transaction) {
        ...
        if (!empty($currencies[$currency_name]->rate)) {
...
function getInvoiceDueAmount($invoice)
{
    ...
    return $invoice->credit != '0.00' ? $i_total - $i_credit : $invoice->total;
}
...
{
    $s = $invoice->cn != '' ? $invoice->cn : $invoice->id;
...
{
    return $user->roleid == 0 ? User::all() : [$user];
}
...
function createPdf($url, $data)
{
    try{

        $response = (new Http())->withOptions([
            'verify' => false
        ])->post(API_BASE_URL.'/pdf', $data);
...
function cloudonex_get_invoice_status_with_badge($status)
{
    global $_L;
    if ($status === 'Paid') {
        return '<span class="badge bg-primary">' . $_L['Paid'] . '</span>';
    }

    if ($status === 'Unpaid') {
        return '<span class="badge bg-danger">' . $_L['Unpaid'] . '</span>';
    }
...
function cloudonex_get_ticket_status_with_badge($status)
{
    ...
    if ($status === 'Open') {
        return '<span class="badge bg-danger">' . $_L['Open'] . '</span>';
    }
...
function cloudonex_get_order_status_with_badge($status)
{
    global $_L;
    if ($status === 'Active') {
        return '<span class="badge bg-success">' . $_L['Active'] . '</span>';
    }

    if ($status === 'Pending') {
        return '<span class="badge bg-danger">' . $_L['Pending'] . '</span>';
    }
...
function sp_purify_data($data)
{
    ...
    $purified_data = [];
    foreach ($data as $key => $value) {
        $purified_data[$key] = is_string($value) ? trim($purifier->purify($value)) : $value;
    }
...
function get_theme_file($file)
{
    ...
    global $config, $base_theme;

    if (!empty($config['frontend_theme']) && file_exists(
        'ui/theme/frontend/' .
        ...
        return '../frontend/' . $config['frontend_theme'] . '/' . $file;
    }
    if (
        file_exists(
            'ui/theme/frontend/' . $base_theme . '/client/' . $file . '.tpl'
        )
    ) {
        return $base_theme . '/client/' . $file;
    }
    if (
        file_exists(
            'ui/theme/default/' . $base_theme . '/client/' . $file . '.tpl'
        )
    ) {
        return $base_theme . '/client/' . $file;
    }
    if (file_exists('ui/theme/default/' . $file . '.tpl')) {
        return $file;
    }
    return $file;
}
...
{
    return (isset($data[$key]) || isset($data->$key)) && $data[$key];
}
...
{
    $log = new Logger('app');
    $log->pushHandler(new StreamHandler('storage/logs/app.log', Logger::ERROR));
    return $log;
}

function getCurrencyConfigJson($currency)
{
    ...
    return json_encode($currency_config);
...
function getClientHomePage($config)
{
    if (isset($config['default_landing_page'])) {
        if($config['default_landing_page'] == 'login'){ # Admin Login
            return 'client';
        }
    }
    return 'client/login';
...
function add_notification_error($message)
{
    $_SESSION['error'] = $message;
}


function app_round($number, $precision = 2)
{
...
function appTwigEnvironment($loader)
{
...
function appPromptBuilder($template, $data = [])
{
    $loader = new FilesystemLoader('system/prompts');
    $twig = appTwigEnvironment($loader);
    return $twig->render($template.'.twig', $data);
}

function appChatAI($config, $prompt, $model = 'gpt-4o-mini')
{
...
function appMarkdownRender($content)
{
    if(empty($content))
    {
        return '';
