...
{block name=head_extras_from_layout}
    <style>
        html body a {
            color: {$config['frontend_link_color']|default:'#7367f0'};;
        }
        .dropdown-item:hover, .dropdown-item:focus {
            color: {$config['frontend_primary_button_hover_color']|default:'#0069d9'};
        }
        .form-control:focus {
            border-color: {$config['frontend_primary_button_hover_color']|default:'#7367f0'};
        }
        .bg-primary {
            background-color: {$config['frontend_primary_button_color']|default:'#7367f0'} !important;
        }
        a:hover {
            color: {$config['frontend_primary_button_hover_color']|default:'#887ef2'};
        }
    </style>
{/block}
{block name="content_body"}
    ...
    <aside class="page-sidebar">
        ...
        <nav id="clx-primary-navigation" class="primary-nav" role="navigation">
            ...
            <ul id="clx-navigation-menu" class="nav-menu">
                ...
                <li {if $selected_navigation eq 'home'}class="active"{/if}><a href="{$_url}client">
                ...
                {if isset($config['kb']) && $config['kb'] == '1'}
    ...
    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
        ...
        <circle fill="#000000" opacity="0.3" cx="18.5" cy="5.5" r="2.5"/>
                ...
                {if !empty($user)}
                    <div>
                        <a href="#" data-bs-toggle="dropdown" title="{$user->account}" class="header-icon d-flex align-items-center justify-content-center ml-2 cursor-pointer">
                            {if $user['img'] eq ''}
                                <img src="{$app_url}ui/lib/img/default-user-avatar.png"  class="profile-image rounded-circle" alt="{$user->account}">
                            {else}
                                <img src="{$app_url}{$user['img']}"  class="profile-image rounded-circle" alt="{$user->account}">
